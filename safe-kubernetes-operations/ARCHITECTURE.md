...
**Companion to:** [Case Study: Making Kubernetes-Gated Support Operations Safe for Non-Kubernetes Engineers](./case-study-rake-console.md)
...
```mermaid
flowchart TD
    A[Engineer runs the script] --> B[Pick cluster/region]
    B --> C[AWS SSO login]
    C --> D[Pick a task from menu]
    D --> E[Auto-discover matching pod]
    E --> F[Preview command + safety checks]
    F -->|destructive task| G[Typed confirmation phrase required]
    F -->|normal task| H[Press Enter to confirm]
    G --> I[Send audit log to Datadog]
    H --> I
    I --> J[kubectl exec into pod]
    J --> K[Pre-filled command on editable line]
    K --> L[Engineer reviews, fills in ID, runs it]
    L --> M[Exit pod session]
    M -->|task has manual follow-up| N[Print post-task alert]
    M --> O[Back at menu: run another / switch region / exit]
```
